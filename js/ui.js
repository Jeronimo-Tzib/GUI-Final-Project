...
export function createUI(eventBus, dataService, rootEl) {
  ...
  function buildRowElement(row) {
    ...
    // Purpose cell (with badge)
    const purposeTd = document.createElement('td');
    const purposeBadge = document.createElement('span');
    purposeBadge.className = 'purpose-badge';
    purposeBadge.setAttribute('data-purpose', row.purpose);
    purposeBadge.textContent = row.purpose;
    purposeTd.appendChild(purposeBadge);
    tr.appendChild(purposeTd);

    // Arrivals cell (numeric, formatted)
    const arrivalsTd = document.createElement('td');
    arrivalsTd.className = 'num';
    arrivalsTd.textContent = formatNumber(row.arrivals);
    tr.appendChild(arrivalsTd);

    // Avg stay nights cell (numeric)
    const stayTd = document.createElement('td');
    stayTd.className = 'num';
    stayTd.textContent = row.avgStayNights;
    tr.appendChild(stayTd);
  ...
  function renderSortIndicators(sortColumn, sortDirection) {
    ...
    // Find the header with matching data-sort-column
    for (const header of els.sortHeaders) {
      if (header.getAttribute('data-sort-column') === sortColumn) {
        if (sortDirection === 'asc') {
          header.classList.add('is-sort-asc');
        } else {
          header.classList.add('is-sort-desc');
        }
        break;
      }
    }
  ...
  function hideDetail() {
    ...
    // Remove highlight from any selected row
  ...
  function wireSubscriptions() {
    ...
    subscribe('view:changed', (payload) => {
      ...
      // Update current page tracking for pagination handlers
      currentPage = page;
      currentPageCount = pageCount;
    });

    // EXTRA CREDIT: row selection events
