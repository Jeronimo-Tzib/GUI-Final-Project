...
export function createDataService(eventBus, dataUrl) {
  ...
  function applyFilters(rows, filters) {
    return rows.filter(row => {
      if (filters.district && row.district !== filters.district) {
        return false;
      }
  ...
  function applySort(rows, sortColumn, sortDirection) {
    ...
    const sorted = rows.slice().sort((a, b) => {
      ...
      if (sortColumn === 'month') {
        aVal = MONTH_ORDER[a.month] || 0;
        bVal = MONTH_ORDER[b.month] || 0;
      }
      ...
      // String comparison for string columns
  ...
  function recomputeAndEmit() {
    ...
    // Mutate state.view.page to clamp it
